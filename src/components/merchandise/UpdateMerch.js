...
function UpdateMerch() {
  ...
  const initialValues = useSelector((state) => state.toUpdateMerch)
  ...
  const handleSubmit = (event) => {
    ...
    if (Object.keys(formError).length === 0) {
      ...
      axios
        .put("http://localhost:8080/api/updateItem", formValue)
        .then((data) => {
          console.log(data);
      ...
      >
        ...
        >
          ...
          <div className="ui form">
            ...
            <button className="btn btn-primary">Update Merchandise</button>
          </div>
        </form>
      </div>
    </div>
